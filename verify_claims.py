...
def run(d):
    ...
    idp = sum(1 for v in pos.values() if v == "IDP")
    check("IDP players are position-resolved for 2023 (else optimal lineups are understated)",
          idp > 50, True, f"{idp} resolved")
    ...
    check("Best lineup efficiency is Denk", (best, eff[best]), ("Denk0", 94.7))
    check("Second best is Johnny", sorted(eff, key=lambda x: -eff[x])[1], "GregoryOdensKnees")
    check("Most flips is Max with 12", (max(flips, key=lambda x: flips[x]),
          max(flips.values())), ("mcryns14", 12))
