...
# The yardage themes, stated here independently of recap_data.py so that this
# file re-derives them rather than agreeing with the code that produced them.
YARDAGE = {
    6:  ("WR", "rec_yd", "most"),
...
def verify(d, pack, tag):
    ...
    if th.get("computable"):
        if wk == 1:
            ...
            # Yardage themes are decided on raw box score yards, not on fantasy
            # points, so they must be re-derived from the stats feed. Checking
            # them against players_points was the original bug here: it went
            # looking for a player who scored 68 fantasy points when 68 was a
            # receiving yard total, found nobody, and blocked a correct pack.
            want_pos, stat_key, mode = YARDAGE[wk]
            sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            import lineup_efficiency as LE
            # pnames, not names. The outer `names` holds the league's owner
            # display names and is used by later checks; shadowing it here made
            # "theme winner is a real owner" compare an owner against a
            # dictionary of NFL players and fail on every yardage week.
            pos, pnames, _ = LE.build_positions(d, sorted(set(LE.seasons(d)) | {y}))
            stats = (load(d, y, "stats") or {}).get(str(wk)) or {}
            check(f"{tag} stats are on file for a yardage theme", bool(stats), True)
            cands = []
            for e in ents:
                for pid in (e.get("starters") or []):
                    if not pid or pid == "0" or pos.get(str(pid)) != want_pos:
                        continue
                    v = (stats.get(str(pid)) or {}).get(stat_key)
                    if v is not None:
                        cands.append((float(v), str(pid), rid.get(e["roster_id"])))
            if mode == "most":
                best = max((c[0] for c in cands), default=None)
                check(f"{tag} theme value is the most {stat_key} by a starting "
                      f"{want_pos}", th["value"], best)
                check(f"{tag} theme winner started that player",
                      th["winner"] in [c[2] for c in cands if c[0] == best], True)
            else:
                # Gronk Week. Closest to 69 either side.
                best = min((abs(c[0] - 69) for c in cands), default=None)
                check(f"{tag} theme value is the closest {want_pos} to 69",
                      round(abs(th["value"] - 69), 4), round(best, 4) if best is not None else None)
                check(f"{tag} theme winner started that player",
                      th["winner"] in [c[2] for c in cands
                                       if abs(c[0] - 69) == best], True)
                if th.get("distance_from_69") is not None:
                    close(f"{tag} distance from 69 is stated correctly",
                          th["distance_from_69"], abs(th["value"] - 69))
            # The named player must be the one carrying that number.
            # Match the same fallback label recap_data uses for a player whose
            # name was never resolved, so an unnamed player does not fail here
            # merely for being unnamed.
            named = [c for c in cands
                     if pnames.get(c[1], f"player#{c[1]}") == th.get("player")
                     and c[2] == th["winner"]]
            check(f"{tag} the named player is the one with that total",
                  bool(named) and named[0][0] == th["value"], True,
                  f"{th.get('player')} {th.get('value')} {stat_key} for {th['winner']}")
        else:
            # Points-based player themes: assert the claimed player really scored
            # the claimed amount for the claimed owner. The eligibility rule
            # differs per week, so this checks the value and ownership rather
