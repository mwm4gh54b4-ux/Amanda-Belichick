...
THE PROSE FILE. prose/<season>-wk<NN>.json, every field optional:

    {
      ...
      "coverage":  "commentary on the outside coverage, if any",
...
def guard_prose(pack, prose):
    """Block any number in the prose the fact pack cannot account for.

    KNOWN LIMIT, stated so nobody mistakes this for more than it is. The guard
    is a whitelist of values, not a proof of provenance. A pack contains a few
    hundred distinct numbers, including most small integers, so a figure lifted
    from a headline can pass simply by coinciding with one of them: "18 red zone
    targets" survives because some player finished 18.0 above replacement.

    What it does catch is every number that does not appear in the pack at all,
    which is the large majority of invented or imported figures, and it catches
    them without exception. What it cannot do is confirm that a number which
    does appear is being used to mean the same thing. Treat it as a floor.

    The defence against the rest is structural rather than mechanical: media
    lives in its own file, is never merged into the pack, and is rendered in a
    section explicitly marked unverified.
    """
...
/* External coverage. Dashed, never solid: the border itself says this was not
   derived from anything and nobody checked it. */
.ext{border:1px dashed var(--rule);border-left:3px dashed var(--muted);
  padding:.6rem .8rem;margin:0 0 .5rem;background:none}
.ext a{color:var(--ink);text-decoration:none;border-bottom:1px solid var(--rule)}
.ext a:hover{border-bottom-color:var(--stamp)}
.ext .src{font-family:"JetBrains Mono",ui-monospace,monospace;font-size:.62rem;
  letter-spacing:.12em;text-transform:uppercase;color:var(--muted);display:block;
  margin-bottom:.2rem}
...
def sec_media(pack, prose, media=None):
    """Outside coverage, quarantined and labelled.

    Nothing here was derived from anything. It is not in the fact pack, its
    numbers are not in the number guard's allowlist, and a writer who repeats a
    figure from a headline as fact will have the page blocked. That is the
    intended behaviour, not a limitation.

    Headline and link only. The excerpt stays in the data file for the writer's
    context and never reaches the page: reproducing a publisher's summary puts
    their copy on your site, whereas linking to it is a citation.
    """
    if not media or not media.get("items"):
        return ""
    out = ['<div class="item bad"><span class="tag">Unverified</span>'
           '<span class="line">Everything in this section is external. None of it '
           'has been checked against anything, and no figure in it is admissible '
           'in the report above.</span></div>']
    for i in media["items"][:8]:
        link = esc(i.get("link") or "")
        title = esc(i.get("title") or "")
        head = f'<a href="{link}" rel="noopener nofollow">{title}</a>' if link else title
        when = (i.get("published") or "")[:10]
        out.append(f'<div class="ext"><span class="src">{esc(i.get("source", ""))}'
                   + (f' &middot; {esc(when)}' if when else "") + "</span>"
                   + head + "</div>")
    return "".join(out) + narr((prose or {}).get("coverage"))
...
SECTIONS = [
    ...
    ("Outside coverage", "Unverified, external", sec_media),
    ("Roast", "Closing statement", sec_roast),
]


def render_page(pack, prose=None, media=None):
    ...
    for name, eyebrow, fn in SECTIONS:
        # sec_media is the only section that sees the media file. Nothing else
        # is given it, and it is never merged into the pack, which is what
        # keeps its numbers out of the guard's allowlist.
        body = fn(pack, prose, media) if fn is sec_media else fn(pack, prose)
        if not body:
            continue
        if name is None:
...
def main():
    ...
    ap.add_argument("--media", help="a pull_media.py output file")
    ...
    media = None
    if a.media and os.path.exists(a.media):
        media = json.load(open(a.media))
        # Deliberately NOT passed to guard_prose. Media numbers must never
        # become admissible simply by having been published somewhere.
        print(f"  media: {len(media.get('items', []))} external items, "
              "excluded from the number guard")
    ...
    for p in paths:
        ...
        with open(os.path.join(a.out, stem + ".html"), "w") as f:
            f.write(render_page(pack, prose, media))
        packs.append(pack)
