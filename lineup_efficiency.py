...
def build_positions(data, years, players_file=None):
    """Position map from draft metadata, then the cached player file, then
    enriched from fixed starter slots, then a conservative flex fallback.

    The fallback matters. FLEX and SUPER_FLEX occupancy does not reveal what a
    player is, so slot inference skips those slots, and a waiver pickup who only
    ever started in a flex slot used to end up with no position and get dropped
    from the optimal lineup pool entirely. That understates the optimal, which
    overstates efficiency, which flatters whoever it happens to.

    So anyone still unresolved is given the eligibility his observed slots
    prove: FLEX occupancy means RB, WR or TE, and SUPER_FLEX occupancy means
    that or QB. He can then fill the flex slot he demonstrably held, and no
    fixed slot he might not qualify for. That is still a lower bound on the true
    optimal, but a much tighter one than dropping him.

    Run pull_players.py to make the fallback mostly unnecessary. Note that the
    cache is used for position and name only: its years_exp field is current
    experience, not experience during a past season, and using it for rookie
    status would misjudge the 2025 Superflex rule.
    """
    pos, names = {}, {}
    for y in years:
        for _, plist in (load(data, y, "picks") or {}).items():
            for p in (plist or []):
                ...
                if pid:
                    ...
                    if nm:
                        names[str(pid)] = nm
    # Cached player file, if one has been pulled. Explicit --players wins, then
    # the conventional location alongside the season data.
    for cand in (players_file, os.path.join(data, "players_nfl.json")):
        if cand and os.path.exists(cand):
            ...
            for pid, rec in cache.items():
                ...
                nm = rec.get("name") or rec.get("full_name")
                if nm and str(pid) not in names:
                    names[str(pid)] = nm
            break
    inferred = 0
    flex_seen = collections.defaultdict(set)
    for y in years:
        lg = load(data, y, "league") or {}
            ...
            for e in entries:
                ...
                for slot, pid in zip(slots, st):
                    ...
                    if str(pid) not in pos:
                        ...
                        inferred += 1
    fallback = 0
    for pid, slots_seen in flex_seen.items():
        if pos.get(pid):
            continue
        if "FLEX" in slots_seen or "REC_FLEX" in slots_seen:
            pos[pid] = "FLEX_ELIG"
        else:
            pos[pid] = "SF_ELIG"
        fallback += 1
    if fallback:
        print(f"  [positions] {fallback} player(s) resolved only to flex "
              f"eligibility. Run pull_players.py to name them properly.")
...
def eligible(slot, p, pid=None, restricted=None):
    if restricted is not None and pid is not None and str(pid) not in restricted:
        return False
    # Players resolved only to flex eligibility. FLEX proves RB, WR or TE;
    # SUPER_FLEX proves that or QB. Neither proves which, so they can hold the
    # flex slot they were seen in and no fixed slot.
    if p == "FLEX_ELIG":
        return slot in ("FLEX", "SUPER_FLEX")
    if p == "SF_ELIG":
        return slot == "SUPER_FLEX"
    if slot == "FLEX":
        return p in FLEX_OK
    if slot == "SUPER_FLEX":
